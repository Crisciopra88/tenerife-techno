...
// Il filtro "venues" restituisce gli eventi di TUTTI i promoter in quel locale
// (così entrano Cru2, NRG, Trick, ecc., non solo Papagayo).
// Per ampliare: aggiungi lo slug-canale in XCEED_CHANNELS o lo slug-locale in XCEED_VENUES.
...
const SOURCES = [
  ...
  // Songkick — concerti/locali non presenti su Xceed (date reali via schema.org/Event)
  { name:"Songkick · Monkey Beach", strategy:"jsonld", url:"https://www.songkick.com/venues/3600314-monkey-beach-club", defaultPromoter:"indie", cat:"Concerto" },
  { name:"Songkick · Magma",        strategy:"jsonld", url:"https://www.songkick.com/es/venues/1896588-magma-arte-and-congresos", defaultPromoter:"blackworks", cat:"Concerto" },
  { name:"Songkick · Tenerife",     strategy:"jsonld", url:"https://www.songkick.com/es/metro-areas/28788-spain-santa-cruz-de-tenerife", defaultPromoter:"indie", cat:"Concerto" },
  // Tomaticket — concerti (reggaeton e non) in Tenerife. IdLugar=51 = Tenerife.
  { name:"Tomaticket · Música",     strategy:"jsonld", url:"https://www.tomaticket.es/etiqueta/musica/?IdLugar=51", defaultPromoter:"indie", cat:"Concerto" },
...
// Scorre una query Xceed (per channel o per venues), paginando, evitando i doppioni per id.
async function fetchXceedList(param, value, seenIds, out) {
  const now = Math.floor(Date.now() / 1000);     // solo eventi futuri/in corso
  const limit = 100;
  let offset = 0;
  for (let page = 0; page < 10; page++) {          // tetto: 10 pagine = 1000 eventi
    let json;
    try {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Se manca la colonna "categoria" in intestazione, la aggiunge da sola.
