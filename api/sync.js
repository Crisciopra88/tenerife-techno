...
const SOURCES = [
  ...
  { name:"Farra / Las Rocas (365top)", strategy:"farra365", url:"https://365top.farra.world", defaultPromoter:"farra", cat:"Club" },
...
async function parse365top(listingHtml, source) {
  ...
  let urls = collect(listingHtml);
  if (!urls.length) {                       // lista vuota a secco -> riprova renderizzando con Jina
    try { urls = collect(await fetchRendered(source.url)); } catch (e) { console.error("365top Jina KO:", e.message); }
  }
  console.log(`365top: ${urls.length} eventi trovati nell'elenco`);
  const out = [];
  let n = 0;
  for (const url of urls) {
    if (n++ >= MAX_PER_SOURCE) break;
    try {
      const ev = parse365topDetail(await fetchText(url), url, source);
      if (ev && ev.nome) out.push(ev);
